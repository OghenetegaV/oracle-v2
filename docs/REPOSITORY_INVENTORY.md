...
| `oracle/application/guide.py`, `alignment.py` | Application | Plain-language review models (queue, cards, view types, sources) and the point-pick alignment geometry (translation + rotation, scale kept, preview without changes) | `session`, `oracle.ui`, tests | Active | RETAIN |
| `oracle/core/clarifications.py` | Core | Engineer clarifications: free-form engineer input kept verbatim (schema 0.5.0) | `project`, `approved`, tests | Active | RETAIN |
| `oracle/ui/review_screen.py`, `point_tools.py`, `details_window.py`, `widgets.py` | UI | The calm review screen, the Align Plan / Split View point-pick tools, the Evidence & Details window and small widgets | `architectural_workspace`, tests | Active | RETAIN |
| `tests/test_engineer_clarifications_core.py`, `test_review_workflow.py`, `test_structural_view_and_tools.py` | Test | Clarifications and schema 0.5.0 (unit); the review workflow (integration); structural rendering, alignment, split preview and view-type correction (integration) | test runner | Active | RETAIN |
| `tests/fixtures/schema_0_4_0_project.json` | Sample Input | A genuine schema-0.4.0 project for the 0.4.0 -> 0.5.0 migration test | `tests/test_engineer_clarifications_core.py` | Reference | RETAIN |
